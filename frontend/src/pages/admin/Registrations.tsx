...
import { Search, Filter, Receipt, User, Calendar, CheckCircle2, XCircle, Clock, Download, ChevronRight, AlertCircle, Save, X as XIcon, Edit3, Trash2 } from 'lucide-react'
...
export default function AdminRegistrations() {
  ...
  const fetchRegistrations = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await client.request(GET_REGISTRATIONS_QUERY, { page: 1, limit: 100 })
  ...
  const filtered = registrations.filter(reg => {
    const matchesSearch = reg.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getRegStatusConfig = (status: string) => {
    switch (status) {
      ...
      default: return { gradient: 'from-purple-500 to-indigo-600', label: status }
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        <div>
          ...
          <p className="text-xl text-gray-600 mt-2">
            Tổng cộng {registrations.length} lượt đăng ký • Doanh thu: {registrations.filter(r => r.paymentStatus === 'paid').reduce((sum, r) => sum + r.paymentAmount, 0).toLocaleString('vi-VN')}đ
          </p>
        </div>
              ...
              <div className="p-8">
                {isEditing ? (
                  <div className="space-y-4">
                    ...
                    <select value={editForm.paymentStatus || ''} onChange={e => setEditForm({ ...editForm, paymentStatus: e.target.value as any })} className="w-full px-4 py-3 border-2 border-indigo-200 rounded-xl focus:border-indigo-500 focus:outline-none">
                    ...
                    <div className="flex gap-3 mt-4">
                      <button onClick={() => saveEdit(reg.id)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition shadow-lg">
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-800">User ID: {reg.userId}</h3>
                        <p className="text-gray-600 text-sm">Event ID: {reg.eventId}</p>
                      </div>
                    </div>
                    ...
                    <div className="mb-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl border border-indigo-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-gray-800">Ngày đăng ký</p>
                          <p className="text-sm text-gray-600">{format(new Date(reg.registrationDate), 'dd/MM/yyyy HH:mm')}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button onClick={() => startEdit(reg)} className="p-3 bg-blue-100 text-blue-600 rounded-xl hover:bg-blue-200 transition shadow-lg">
                          <Edit3 className="w-5 h-5" />
                        </button>
                        <button onClick={() => handleDelete(reg.id)} className="p-3 bg-red-100 text-red-600 rounded-xl hover:bg-red-200 transition shadow-lg">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button onClick={() => handleQuickAction(reg.id, 'status', 'confirmed')} className="btn btn-success btn-sm">
                        Xác nhận
                      </button>
                      <button onClick={() => handleQuickAction(reg.id, 'paymentStatus', 'paid')} className="btn btn-outline btn-success btn-sm">
                        Đã thanh toán
                      </button>
                    </div>
                  </>
